...
if __name__ == "__main__":
    ...
    prod = spark.read.parquet('hdfs://localhost:9000/test/prod2/') \
        ...
        .alias('prod').repartition(600)    # cnt : 46271
    ...
    self_prod = self_prod\
        ...
        .withColumn('Jaccard_Similarity', get_jaccard_sim(F.col('prod_nm_tkns_1'), F.col('prod_nm_tkns_2'))).where(F.col('Jaccard_Similarity') > 0.22)\
        # .withColumn(
        #     '_rnk',
    ...
    # self_prod.write.format("parquet").mode("append").save("hdfs://localhost:9000/jaccard_similarity")

    # self_prod = spark.read.parquet("hdfs://localhost:9000/jaccard_similarity")
    # self_prod.where(F.col('Jaccard_Similarity') >= 0.2).orderBy(F.col('prod_nm_1').desc()).show(1000, False)
    ...
    #     .select(
    #         # F.col('_c2'),
    #         F.regexp_replace(F.lower(F.col('_c2')),  "[^ㄱ-힝0-1a-z ]", '').alias('shipping_nm')
    #     ).where((F.col('shipping_nm').isNotNull()) & (F.length(F.col('shipping_nm'))) > 1).alias('shipping_nm')
    # shipping_nm.select(F.count(F.col('shipping_nm'))).show()
    # prod.select(F.count(F.col('prod_nm'))).show()
    #
    # # ship_prod = prod.join(
    # #     shipping_nm,
    # #     F.col('shipping_nm.shipping_nm') != F.col('prod.prod_nm'), 'full'
    # # ).select(
    # #     F.col('prod.prod_nm').alias('prod_nm_1'),
    # #     F.col('prod.prod_nm_tkns').alias('prod_nm_tkns_1'),
    # #     shipping_nm['*']
    # # ).alias('ship_prod')
    # #
    # # ship_prod.show(1000, False)
